...
class ArtifactWriter:
    def __init__(self, out_path: ArtifactPath, fps: float, save_artifacts: bool = True):
        self.out_path = out_path
        self.save_artifacts = save_artifacts
        
        self.pose_list: list[tuple[int, SE3]] = []
        self.intrinsics_list: list[tuple[int, torch.Tensor]] = []
        self.camera_type_list: list[tuple[int, CameraType]] = []
        self.instance_phrases_combined: dict[int, str] = {}
        
        self.rgb_writer: VideoWriter | None = None
        self.depth_zip: zipfile.ZipFile | None = None
        self.mask_zip: zipfile.ZipFile | None = None
        
        if self.save_artifacts:
            self.rgb_writer = VideoWriter(out_path.rgb_path, fps)
            self.out_path.depth_path.parent.mkdir(exist_ok=True, parents=True)
            self.depth_zip = zipfile.ZipFile(out_path.depth_path, "w", zipfile.ZIP_DEFLATED)
            self.out_path.mask_path.parent.mkdir(exist_ok=True, parents=True)
            self.mask_zip = zipfile.ZipFile(out_path.mask_path, "w", zipfile.ZIP_DEFLATED)
    ...
    def write(self, frame_idx: int, frame: VideoFrame):
        ...
        # RGB
        if self.rgb_writer:
            self.rgb_writer.write((frame.rgb.cpu().numpy() * 255).astype(np.uint8))
        
        # Pose
        if frame.pose is not None:
            self.pose_list.append((frame_idx, frame.pose))
            
        # Intrinsics
        if frame.intrinsics is not None:
            self.intrinsics_list.append((frame_idx, frame.intrinsics))
            
        # Camera Type
        if frame.camera_type is not None:
            self.camera_type_list.append((frame_idx, frame.camera_type))

        # Depth
        if frame.metric_depth is not None and self.depth_zip:
            metric_depth = frame.metric_depth.cpu().numpy()
            height, width = metric_depth.shape
            header = OpenEXR.Header(width, height)
            header["channels"] = {"Z": Imath.Channel(Imath.PixelType(Imath.PixelType.HALF))}
            with tempfile.NamedTemporaryFile(suffix=".exr") as f:
                ...
                exr.writePixels({"Z": metric_depth.astype(np.float16).tobytes()})
                exr.close()
                self.depth_zip.write(f.name, f"{frame_idx:05d}.exr")

        # Mask
        if frame.instance is not None and self.mask_zip:
            _, mask_buffer = cv2.imencode(".png", frame.instance.cpu().numpy().astype(np.uint8))
            self.mask_zip.writestr(f"{frame_idx:05d}.png", mask_buffer.tobytes())
            
        # Phrases
        if frame.instance_phrases is not None:
            self.instance_phrases_combined.update(frame.instance_phrases)

    def close(self):
        if self.rgb_writer:
            self.rgb_writer.__exit__(None, None, None)
        ...
        if len(self.pose_list) > 0:
            pose_data = np.stack([pose.matrix().cpu().numpy() for _, pose in self.pose_list], axis=0)
            pose_inds = np.array([idx for idx, _ in self.pose_list])
            self.out_path.pose_path.parent.mkdir(exist_ok=True, parents=True)
            np.savez(self.out_path.pose_path, data=pose_data, inds=pose_inds)
            
        if len(self.intrinsics_list) > 0:
            intrinsics_data = np.stack([intr.cpu().numpy() for _, intr in self.intrinsics_list], axis=0)
            intrinsics_inds = np.array([idx for idx, _ in self.intrinsics_list])
            self.out_path.intrinsics_path.parent.mkdir(exist_ok=True, parents=True)
            np.savez(self.out_path.intrinsics_path, data=intrinsics_data, inds=intrinsics_inds)
            
        if len(self.camera_type_list) > 0:
            self.out_path.camera_type_path.parent.mkdir(exist_ok=True, parents=True)
            with self.out_path.camera_type_path.open("w") as f:
                for idx, ctype in self.camera_type_list:
                    f.write(f"{idx}: {ctype.name}\n")
                    
        if len(self.instance_phrases_combined) > 0:
            self.out_path.mask_phrase_path.parent.mkdir(exist_ok=True, parents=True)
            with self.out_path.mask_phrase_path.open("w") as f:
                for idx, phrase in self.instance_phrases_combined.items():
                    f.write(f"{idx}: {phrase}\n")

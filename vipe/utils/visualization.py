...
class VisualizationWriter:
    def __init__(
        ...
        self.fps = fps
        self.img_h, self.img_w = frame_size
        self.img_h //= subsample_factor
        self.img_w //= subsample_factor
        self.slam_output = slam_output
        self.subsample_factor = subsample_factor
        self.attributes = attributes
        
        self.vw_context = VideoWriter(video_path, fps)
        self.vw = self.vw_context.__enter__()
        
        # Prepare PCD point cloud once if available
        self.pcd_xyz = None
        self.pcd_rgb = None
        if self.slam_output is not None:
             slam_map = unpack_optional(self.slam_output.slam_map)
             self.pcd_xyz = slam_map.dense_disp_xyz.cpu().numpy()
             self.pcd_rgb = slam_map.dense_disp_rgb.cpu().numpy()
             
        self.na_img = np.zeros((self.img_h, self.img_w, 3), dtype=np.uint8)
        text_img = image.text("N/A")
        self.na_img = image.place_image(
            text_img,
            self.na_img,
            self.img_w // 2 - text_img.shape[1] // 2,
            self.img_h // 2 - text_img.shape[0] // 2,
        )
        self.na_img = (self.na_img[..., :3] * 255).astype(np.uint8)
    ...
    def get_rgb_img(self, frame: VideoFrame) -> np.ndarray:
        rgb_img = frame.rgb.cpu().numpy().astype(float)
        rgb_img = (rgb_img * 255).astype(np.uint8)
        rgb_img = cv2.resize(rgb_img, (self.img_w, self.img_h))
        return rgb_img

    def get_depth_img(self, frame: VideoFrame) -> np.ndarray:
        if (depth_data := frame.metric_depth) is None:
            return self.na_img
        
        depth_data = depth_data.reciprocal()
        # Per frame normalization
        valid_mask = ~frame.sky_mask & torch.isfinite(depth_data)
        if valid_mask.sum() == 0:
             return self.na_img
             
        valid_depth = depth_data[valid_mask]
        depth_min = valid_depth.min().item()
        depth_max = valid_depth.max().item()

        depth_data[frame.sky_mask] = depth_min
        depth_data[~torch.isfinite(depth_data)] = depth_min
        
        depth_data = depth_data[::self.subsample_factor, ::self.subsample_factor]
        depth_img = depth_data.cpu().numpy().astype(float)
        ...
        depth_img = np.clip(depth_img, 0, 1)
        return colorize_depth(depth_img)

    def get_pcd_img(self, frame: VideoFrame, rgb_img: np.ndarray) -> np.ndarray:
        if self.pcd_xyz is None:
            return self.na_img
            
        intrinsics = unpack_optional(frame.intrinsics)
        if torch.sum(intrinsics) < 1e-6:
                pcd_img = project_points_panorama(
                    self.pcd_xyz,
                    frame.pose,
    ...
    def get_instance_img(self, frame: VideoFrame, rgb_img: np.ndarray) -> np.ndarray:
        if frame.instance is None:
            return self.na_img
            
        instance_img = (inst_np := frame.instance.cpu().numpy()).astype(float)
        instance_img = colorize_mask(instance_img)

        if frame.instance_phrases is not None:
                for instance_id, instance_phrase in frame.instance_phrases.items():
                    if instance_id <= 0:
                        continue
                    text_img = image.text(instance_phrase)
                    inst_mask = inst_np == instance_id
        ...
        if instance_img.dtype == np.float64:
             instance_img = (instance_img * 255).astype(np.uint8)
        
        instance_img = cv2.resize(instance_img, (self.img_w, self.img_h))
        return cv2.addWeighted(rgb_img, 0.5, instance_img, 0.5, 0)
        
    def get_rectified_img(self, frame: VideoFrame) -> np.ndarray:
        # We need to compute coords_norm once
        if self.coords_norm is None:
             original_intr = frame.camera_type.build_camera_model(frame.intrinsics).scaled(
                1 / self.subsample_factor
             )
             pinhole_intr = original_intr.pinhole()
             device = pinhole_intr.intrinsics.device
             y, x = torch.meshgrid(torch.arange(self.img_h).float(), torch.arange(self.img_w).float(), indexing="ij")
             y, x = y.to(device), x.to(device)
             pts, _, _ = pinhole_intr.iproj_disp(torch.ones_like(x), x, y)
             coords, _, _ = original_intr.proj_points(pts)
             coords_norm = 2.0 * coords / torch.tensor([self.img_w, self.img_h], device=coords.device) - 1.0
             self.coords_norm = coords_norm.reshape(1, self.img_h, self.img_w, 2)
        
        img = frame.rgb.permute(2, 0, 1).unsqueeze(0)
        img = torch.nn.functional.grid_sample(
            img,
            self.coords_norm,
            mode="bilinear",
    ...
    def write(self, frame_idx: int, frame: VideoFrame):
        rgb_img = self.get_rgb_img(frame)
        
        img_rows = []
        for t_arr in self.attributes:
            img_row = []
            for t in t_arr:
                 if t == "rgb":
                     img_row.append(rgb_img)
                 elif t == "depth":
                     img_row.append(self.get_depth_img(frame))
                 elif t == "pcd":
                     img_row.append(self.get_pcd_img(frame, rgb_img))
                 elif t == "instance":
                     img_row.append(self.get_instance_img(frame, rgb_img))
                 elif t == "rectified":
                     img_row.append(self.get_rectified_img(frame))
                 elif t == "empty":
                     img_row.append(self.na_img)
            img_rows.append(np.concatenate(img_row, axis=1))
        ...
        if frame.intrinsics is not None:
             focal = frame.intrinsics[0].item()
             if focal > 1e-6:
                 fov_y = 2 * np.arctan(frame.size()[0] / (2 * focal))
                 fov_y = np.rad2deg(fov_y)
                 text_desc += f" | fovY {fov_y:.2f}"
        
        current_pose = frame.pose
        if self.last_pose is not None and current_pose is not None:
             self.trajectory_length += np.linalg.norm((self.last_pose.inv() * current_pose).translation()[:3].cpu().numpy())
        if current_pose is not None:
             self.last_pose = current_pose
        
        text_desc += f" | Traj {self.trajectory_length:.4f}"
        if len(frame.information) > 0:
             text_desc += f" | {frame.information}"
    ...
    def close(self):
        self.vw_context.__exit__(None, None, None)
